...
export default function UpCommingShipments() {
  ...
  // Filter only shipments with the selected statuses
  const filteredData = data?.data?.filter(
    (item: ApiShipment) =>
      item.status === 'AWAITING_PICKUP' ||
      item.status === 'IN_TRANSIT' ||
      item.status === 'CUSTOMS_CLEARANCE'
  ) || [];
  ...
  return (
    <div className="min-h-screen bg-gray-50 mx-auto">
      ...
      <main className="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-8">
          <h2 className="text-3xl font-bold text-gray-900 mb-2">Upcoming Shipments</h2>
